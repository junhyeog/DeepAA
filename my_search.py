...
parser.add_argument(
    "--clip_policy_gradient_norm", default=5.0, type=float, help="clipping the policy gradient by norm"
)
...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="use batchnorm for policy search, Default to False",
)
...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="repeat random operations (randCrop, randFlip, randCutout",
)
...
with mirrored_strategy.scope():
    ...
    train_loss_fun = tf.keras.losses.SparseCategoricalCrossentropy(
        from_logits=True, reduction=tf.keras.losses.Reduction.NONE
    )
    optim_net = get_optim_net(args, nb_train_steps)
assert (
    args.train_same_labels % mirrored_strategy.num_replicas_in_sync == 0
), "Make sure val_same_labels can be divided by num_replicas_in_sync"
...
def get_pretrain_data():
    ...
    try:
        ...
        images, labels = next(train_ds_iter)
    bs = len(labels)
    images, _ = augmentation_default(images, labels, [None] * bs, [None] * bs, use_post_aug=True, pool=pool)
    return tf.convert_to_tensor(images, dtype=tf.float32), tf.convert_to_tensor(labels, tf.int32)


@tf.function(
    input_signature=[
        tf.TensorSpec(shape=(None, *args.img_size), dtype=tf.float32),
...
def pretrain():
    for epoch in range(args.nb_epochs):
        ...
        for bno in range(nb_train_steps):
            ...
            loss, labels_pred = train_step(images, labels, clip_gradient_norm=5.0)
            train_loss(loss)  # only record the last method's loss and accuracy
            train_accuracy(labels, labels_pred)
            pbar.update(bno + 1)

    print(
        "Saving the checkpoint to {}".format(
            "./results/images/ckpt{}/model_ckpt{}".format(os.environ["CUDA_VISIBLE_DEVICES"], epoch - 1)
        )
    )
    ...
    model.save_weights(
        "./results/images/ckpt{}/model_ckpt{}".format(os.environ["CUDA_VISIBLE_DEVICES"], args.nb_epochs)
    )
    model.load_weights(
        "./results/images/ckpt{}/model_ckpt{}".format(os.environ["CUDA_VISIBLE_DEVICES"], args.nb_epochs)
    )
...
search_summary_writer = tf.summary.create_file_writer(
    "./results/images/logs/cuda{}/search".format(os.environ["CUDA_VISIBLE_DEVICES"])
)
graph_summary_writer = tf.summary.create_file_writer(
    "./results/images/logs/cuda{}/graph".format(os.environ["CUDA_VISIBLE_DEVICES"])
)
...
if __name__ == "__main__":
    ...
    for layer in model.layers:
        if isinstance(layer, tf.keras.layers.experimental.SyncBatchNormalization) or isinstance(
...
@tf.function
def policy_gradient_stage1(reduce_random_mat, images_aug, labels_aug, images_val, labels_val, weight_1, weights_2):
    ...
    reduce_random_mat = tf.squeeze(reduce_random_mat)  # ! [T, T], np.eye(T)
    images_aug = tf.squeeze(images_aug)  # ! [T, *img_size]
    labels_aug = tf.squeeze(labels_aug)  # ! [T]
    images_val = tf.squeeze(images_val)  # ! [val_bs, *img_size]
    labels_val = tf.squeeze(labels_val)  # ! [val_bs]
    weight_1 = tf.squeeze(weight_1)  # ! [], = 1
    weights_2 = tf.squeeze(weights_2)  # ! [T], = p_theta

    bs = _PARALLEL_BATCH  # ! 16 or 128 or 256
    ...
    def one_batch_grad(imgs, labs, w1, w2, grad):
        # ! imgs: [bs, *img_size], labs: [bs], w1: [], w2: [bs]
    ...
    def cal_grad(imgs, labs, w1, w2):
        """
        - imgs를 _PARALLEL_BATCH 단위로 나눠서 grad를 계산하고 그 합을 출력
        - 한 _PARALLEL_BATCH의 grad는 step2_cal_JVP_vStep 함수로 계산
        - step2_cal_JVP_vStep 함수는 tf.GradientTape로 grad를 계산
        ...
        Args:
            imgs (_type_): [num_data, *img_size]
            labs (_type_): [num_data]
            w1 (_type_): [1]
            w2 (_type_): [num_data]
        ...
        """
        L = tf.shape(imgs)[0]  # ! num_data
        grad0 = tf.nest.map_structure(lambda g: tf.zeros_like(g), model.trainable_variables)
        grad, _ = tf.while_loop(
            cond=lambda grad_acc, k: tf.cast(k, dtype=tf.int32)
            < tf.cast(tf.math.ceil(tf.cast(L, dtype=tf.float32) / tf.cast(bs, dtype=tf.float32)), dtype=tf.int32),
            # ! cond = k < ceil(num_data/_PARALLEL_BATCH)
            body=lambda grad_acc, k: (
                one_batch_grad(  # ! calculate gradients of one _PARALLEL_BATCH
                    ...
                    w1,  # ! []
                    w2[batching(L, bs, k)[0] : batching(L, bs, k)[1]],  # ! [_PARALLEL_BATCH]
                    grad_acc,
                ),
                k + 1,
            ),
    ...
    grad_val = cal_grad(
        images_val,  # ! [val_bs, *img_size]
        labels_val,  # ! [val_bs]
        tf.constant(1.0, dtype=tf.float32),  # ! [], = 1
        tf.ones(val_bs, dtype=tf.float32) / tf.cast(val_bs, dtype=tf.float32),  # ! [val_bs], = [1 / val_bs, ...]
    )
...
def policy_gradient_stage2(
    ...
    grad_val = cal_grad(
        ...
        tf.ones(val_bs, dtype=tf.float32) / tf.cast(val_bs, dtype=tf.float32),
    )
...
def distributed_train_stage1(dist_inputs):
    per_replica_cos_sim, per_replica_grad_importance = mirrored_strategy.run(
        policy_gradient_stage1, args=(*dist_inputs,)
    )
    return mirrored_strategy.experimental_local_results(
        per_replica_cos_sim
    ), mirrored_strategy.experimental_local_results(per_replica_grad_importance)


@tf.function
def distributed_train_stage2(dist_inputs):
    per_replica_cos_sim, per_replica_grad_importance = mirrored_strategy.run(
        policy_gradient_stage2, args=(*dist_inputs,)
    )
    return mirrored_strategy.experimental_local_results(
        per_replica_cos_sim
    ), mirrored_strategy.experimental_local_results(per_replica_grad_importance)
...
def train_policy_stage1(stage, images_val_, labels_val_, images_batch, labels_batch):
    ...
    # ! images_val_ : [search_bs, val_bs, *img_size]
    # ! labels_val_ : [search_bs, val_bs]
    # ! images_batch : [search_bs, 1, *img_size]
    # ! labels_batch : [search_bs, 1]

    EXP = 1  # expansion factor
    images_val_, labels_val_ = augmentation_test(
        sum(images_val_, []),  # ! : [search_bs * val_bs, *img_size]
        np.concatenate(labels_val_),  # ! : [search_bs * val_bs]
        np.array([[0]] * search_bs * val_bs, dtype=np.int32),  # ! : [search_bs * val_bs, 1]
        np.array([[0]] * search_bs * val_bs, dtype=np.float32) / float(args.l_mags - 1),  # ! : [search_bs * val_bs, 1]
        use_post_aug=True,
        pool=pool,
        chunksize=args.chunk_size,
    )
    # ! images_val_ : [search_bs, val_bs, *img_size] -> [search_bs * val_bs, *img_size]
    # ! labels_val_ : [search_bs, val_bs] -> [search_bs * val_bs]

    images_val_ = np.reshape(images_val_, [search_bs, val_bs, *args.img_size])
    labels_val_ = np.reshape(labels_val_, [search_bs, val_bs])
    # ! images_val_ : [search_bs * val_bs, val_bs, *img_size] -> [search_bs, val_bs, *img_size]
    # ! labels_val_ : [search_bs * val_bs] -> [search_bs, val_bs]

    images_batch = repeat(images_batch, EXP, axis=0)
    labels_batch = repeat(labels_batch, EXP, axis=0)
    # ! images_batch : [search_bs, 1, *img_size] -> [search_bs * EXP, 1, *img_size]
    # ! labels_batch : [search_bs, 1] -> [search_bs * EXP, 1]

    ops_dense, mags_dense, reduce_random_mat, ops_mags_idx, probs, probs_exp = all_using_policies[
        stage - 1
    ].get_dense_aug(
        None, args.repeat_random_ops
    )  # None, False
    # ! reduce_random_mat : [T, T], np.eye(T)
    # ! probs : [T], softmax된 값
    # ! probs_exp == probs : [T], softmax된 값
    ...
    # ! images_aug_last : [search_bs * EXP, T, *img_size]
    # ! labels_aug_last : [search_bs * EXP, T]
    ...
    all_using_optim_policies[stage - 1].apply_gradients(
        zip(grad_policy, all_using_policies[stage - 1].trainable_variables)
    )
    del tape
...
def train_policy_stage2(stage, images_val_, labels_val_, images_batch, labels_batch):
    ...
    for k_stage in range(1, stage + 1):
        ...
        ops_s_, mags_s_, ops_mags_idx_s, probs_sample = all_using_policies[k_stage - 1].sample(
            dummy_images, dummy_images, None, augNum=1
        )
        ops_s.append(ops_s_)
        mags_s.append(mags_s_)
    ops_s = np.concatenate(ops_s, axis=1)
    mags_s = np.concatenate(mags_s, axis=1)
    images_aug_s, labels_aug_s = augmentation_search(
        sum(images_aug_s, []),
        np.concatenate(labels_aug_s, axis=0),
        ops_s,
        mags_s.astype(np.float32) / float(args.l_mags - 1),
        use_post_aug=False,
        pool=pool,
        chunksize=None,
    )
    ...
    for k_stage in range(1, stage):
        ...
        ops_k_, mags_k_, ops_mags_idx_k, probs_sample = all_using_policies[k_stage - 1].sample(
            dummy_images, dummy_images, None, augNum=1
        )
        ops_k.append(ops_k_)
    ...
    images_aug_k, labels_aug_k = augmentation_search(
        sum(images_aug_k, []),
        np.concatenate(labels_aug_k, axis=0),
        ops_k,
        mags_k.astype(np.float32) / float(args.l_mags - 1),
        use_post_aug=False,
        pool=pool,
        aug_finish=False,
        chunksize=args.chunk_size,
    ...
    ops_dense, mags_dense, reduce_random_mat, ops_mags_idx, probs, probs_exp = all_using_policies[
        stage - 1
    ].get_dense_aug(None, repeat_random_ops=args.repeat_random_ops)
    images_aug_k, labels_aug_k = augmentation_search(
        repeat(images_aug_k, len(ops_dense), axis=0),
        np.repeat(labels_aug_k, len(ops_dense), axis=0),
        np.tile(ops_dense, [search_bs * EXP_G, 1]),
        np.tile(mags_dense, [search_bs * EXP_G, 1]).astype(np.float32) / float(args.l_mags - 1),
        use_post_aug=False,
        pool=pool,
        chunksize=None,
    )
    ...
    grad_importance = tf.reduce_mean(grad_importance.numpy(), axis=0) - tf.math.reduce_std(
        grad_importance.numpy(), axis=0
    )
