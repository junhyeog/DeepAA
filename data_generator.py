...
class DataGenerator(Sequence):
    ...
    def sample_labeled_data_batch(self, label, bs):
        ...
        return [self.data[k] for k in data_indices], np.array(
            [self.labels[k] for k in data_indices], dtype=self.labels[0].dtype
        )
...
class DataAugmentation(object):
    ...
    def sequantially_augment(self, args):
        img_idx, img_, op_idxs_, mags_, aug_finish = args
        ...
        if self.ops is not None:
            for op_idx, mag in zip(op_idxs_, mags_):
                op, minval, maxval = self.ops[op_idx]
                assert mag > -1e-5 and mag < 1.0 + 1e-5, "magnitudes should be in the range of (0., 1.)"
    ...
    def crop_IMAGENET(self, img):
        ...
        if isinstance(img, np.ndarray):
            assert (
                img.shape == (IMAGENET_SIZE[1], IMAGENET_SIZE[0], 3) and img.dtype == np.uint8
            ), "numpy array should be {}, but got {}. crop_IMAGENET does not apply to numpy array, but got {}".format(
                IMAGENET_SIZE, img.size, img.dtype
            )
            return img
        w, h = img.size
    ...
    def __call__(
        self, images, labels, samples_op, samples_mag, use_post_aug, pool=None, chunksize=None, aug_finish=True
    ):
        # ! images: [len_img, *img_size]
        # ! labels: [len_img]
        # ! samples_op: [len_img, num_ops]
        # ! samples_mag: [len_img, num_ops]
        # ! => 1개의 img batch가 들어와서 각각의 img에 대해 augmentation을 수행
        self.check_data_type(images, labels)

        self.use_post_aug = use_post_aug
        self.batch_len = len(labels)
        if aug_finish:
            aug_imgs = np.empty([self.batch_len, *self.image_shape], dtype=np.float32)  # [batch, *img_size]
        else:
